...
setwd("C:/Users/rsoren/Dropbox/projects/fbd/notebooks/inla-apc/fbd-version/")
...
cds <- subset(country_codes.dat, location_id %in% unique(df_pop$location_id),
  select = c(iso3, location_id, location_name, gbd_analytical_region_id, gbd_analytical_region_name)
)

# pick locations/sexes
# iso <- c('USA', 'CAN', 'GBR')
# sex <- c(1)
...
death_data <- lapply(death_data, FUN = function(x) apply(x, 2, function(y) round(y / 1000)))
pop_data <- lapply(pop_data, FUN = function(x) apply(x, 2, function(y) round(y / 1000)))
...
save(inla_fit, file=file.path('results_by_region', paste0('inla_fit_AC_nSmokers_yr27_region', reg, '.RData')))
...
save(results, file=file.path('results_by_region', paste0('predictions_AC_nSmokers_yr27_region', reg, '.RData')))
write.csv(results, file=file.path('results_by_region', paste0('predictions_AC_nSmokers_yr27_region', reg, '.csv')), row.names=FALSE)
